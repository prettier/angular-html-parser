...
import {getNsPrefix, mergeNsAndName, splitNsName, TagContentType, TagDefinition} from './tags';
import {AttributeNameToken, AttributeQuoteToken, BlockCloseToken, BlockOpenStartToken, BlockParameterToken, CdataStartToken, CommentStartToken, ExpansionCaseExpressionEndToken, ExpansionCaseExpressionStartToken, ExpansionCaseValueToken, ExpansionFormStartToken, IncompleteBlockOpenToken, IncompleteLetToken, IncompleteTagOpenToken, InterpolatedAttributeToken, InterpolatedTextToken, LetEndToken, LetStartToken, LetValueToken, TagCloseToken, TagOpenStartToken, TextToken, Token, TokenType} from './tokens';
...
export class Parser {
  ...
  parse(
    ...
    const getTagDefinition =
        isTagNameCaseSensitive ? this.getTagDefinition : lowercasify(this.getTagDefinition);
    const getDefaultTagContentType = (tagName: string) =>
...
class _TreeBuilder {
  ...
  private _consumeCdata(startToken: CdataStartToken) {
    const text = this._advance<TextToken>();
    const value = this._getText(text);
    const endToken = this._advanceIf(TokenType.CDATA_END);
    this._addToParent(new html.CDATA(
        value, new ParseSourceSpan(startToken.sourceSpan.start, (endToken || text).sourceSpan.end),
        [text]));
  }

  private _consumeComment(startToken: CommentStartToken) {
  ...
  private _consumeDocType(startToken: Token) {
    const text = this._advanceIf(TokenType.RAW_TEXT);
  ...
  private _parseExpansionCase(): html.ExpansionCase|null {
    ...
    const expansionCaseParser = new _TreeBuilder(
        exp, this.getTagDefinition, this.canSelfClose, this.allowHtmComponentClosingTags,
  ...
  private _consumeStartTag(startTagToken: TagOpenStartToken|IncompleteTagOpenToken) {
    ...
    const startSpan = new ParseSourceSpan(
        startTagToken.sourceSpan.start, end, startTagToken.sourceSpan.fullStart);
    const nameSpan =
        new ParseSourceSpan(startTagToken.sourceSpan.start.moveBy(1), startTagToken.sourceSpan.end);
  ...
  private _consumeAttr(attrName: AttributeNameToken): html.Attribute {
    ...
    let startQuoteToken: AttributeQuoteToken|undefined;

    // Consume any quote
    if (this._peek.type === TokenType.ATTR_QUOTE) {
      startQuoteToken = this._advance<AttributeQuoteToken>();
    }
    ...
    const valueSpan = valueStartSpan && valueEnd &&
        new ParseSourceSpan(
                          startQuoteToken?.sourceSpan.start ?? valueStartSpan.start, valueEnd,
